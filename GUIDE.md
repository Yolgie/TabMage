# Yolgie's Tab Mage - Local Testing & Packaging Guide
...
```bash
mkdir -p dist
zip -r dist/yolgies-tab-mage.zip manifest.json background.js popup
```

For Firefox, an `.xpi` is essentially a zip archive with a different extension. You can copy/rename the output if needed:

```bash
cp dist/yolgies-tab-mage.zip dist/yolgies-tab-mage.xpi
```
